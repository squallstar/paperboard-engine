...
class Instagram {
  ...
  public function __construct($options = null) {
    $this->setApiKey($this->config->item('instagram_consumer_key'));
    $this->setApiSecret($this->config->item('instagram_consumer_secret'));
    if (is_array($options) && isset($options['callback']))
    {
      $this->setApiCallback($options['callback']);
    }
  }
  ...
  public function getUserFollows($id = 'self', $limit = 0) {
    $resp = $this->_makeCall('users/' . $id . '/follows', true, array('count' => $limit));
